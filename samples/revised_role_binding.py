...
from typing import Any, Callable, Generator, Optional, ClassVar, TypeVar, List
from dataclasses import dataclass, field

from neighborly.simulation import Neighborly
from neighborly.core.roles import RoleList, Role
...
class RoleCastingContext:
    ...
    bindings: RoleList = field(default_factory=RoleList)
...
EventRoleConsideration = Callable[[GameObject, _T], Optional[float]]


class EventRoleConsiderationList(List[EventRoleConsideration[_T]]):
...
def event_role(
    name: str = "", considerations: Optional[EventRoleConsiderationList[_T]] = None
):
    """A decorator to indicate that a function is for casting a role"""

    def wrapper(fn: RoleCastingFn):
        return EventRole(
...
def _update_roles(cls: type) -> None:
    """
    Upatees the internal records of event roles for classes that detive from the
    ...
    """
    event_roles: list[EventRole] = []

    # Check the existing event role methods of the parents
    for scls in cls.__bases__:
        for entry in getattr(scls, "__event_roles__", ()):
            if isinstance(entry, EventRole):
                event_roles.append(entry)

    # Check the declared event role methods
    for _, value in cls.__dict__.items():
        if isinstance(value, EventRole):
            event_roles.append(value)

    # Set combined colllection of event roles for the given type
...
class RandomEvent(metaclass=RandomEventMeta):
    ...
    __event_roles__: ClassVar[list[EventRole]] = []
    """All role binfing functions"""
    ...
    def __init__(self, timestamp: SimDateTime, roles: RoleList) -> None:
    ...
    def get_probability(self) -> float:
        ...
        for event_role in self.__event_roles__:
            if event_role.considerations:
                for gameobject in self.roles.get_all(event_role.name):
                    probability *= event_role.considerations.calculate_score(
                        gameobject, self
                    )
                    contributer_count += 1

        if contributer_count > 0:
            # Take the arithmetic mean of the contributing probability scores
            return probability ** (1 / contributer_count)
        else:
            return 0.5
    ...
    def instantiate(
        ...
        if len(cls.__event_roles__) == 0:
            yield cls(world.get_resource(SimDateTime).copy(), RoleList())
            return
        ...
        ctx = RoleCastingContext(
            world=world, bindings=bindings if bindings else RoleList()
        )
        ...
        while True:
            try:
                if len(current_state.pending) > 0:
                    ...
                    new_bindings = RoleList(
                        [
                            *current_state.ctx.bindings,
                            *[
                                Role(current_state.role_to_cast.name, r)
...
class StartHeroStory(RandomEvent):
    """Begins a story of a hero versus a villian."""

    @event_role(
        name="hero",
        considerations=EventRoleConsiderationList([lambda gameobject, event: 0.8]),
    )
    @staticmethod
    def hero(ctx: RoleCastingContext) -> Generator[tuple[GameObject, ...], None, None]:
        """Bind a hero."""
