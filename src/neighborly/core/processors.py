...
class CharacterProcessor(esper.Processor):
    ...
    def process(self, *args, **kwargs):
        ...
        for character_id, character in self.world.get_component(GameCharacter):
            ...
            if character.age >= character.max_age and character.config.lifecycle.can_die:
                self._die(character_id, character)

    def _grow_older(self, character_id: int, character: GameCharacter, hours: float) -> None:
    ...
    def _simulate_dating(self, character_id: int, character: GameCharacter) -> None:
        ...
        person_to_ask = behavior_utils.get_character(
            self.world, person_to_ask_id)

        # They will accept if the other person is also one of their love interests
        if character_id in person_to_ask.relationships.get_with_flags(Connection.LOVE_INTEREST) \
                and person_to_ask.relationships.significant_other == None:
    ...
    def _simulate_breakup(self, character_id: int, character: GameCharacter) -> None:
        ...
        if character.relationships.significant_other:
            if not character.relationships[character.relationships.significant_other].has_flags(Connection.LOVE_INTEREST):
                # Break up if dating
                if character.statuses.has_status("dating"):
                    partner_id = character.relationships.significant_other
                    partner = behavior_utils.get_character(
                        self.world, partner_id)
                    ...
                    partner.relationships[character_id].add_modifier(
                        RelationshipModifier("break up", salience=5, spark=-10, charge=-5))
        ...
        if character.location:
            ...
            for other_character_id in location.characters_present:
                ...
                if other_character_id not in character.relationships:
                    ...
                    character.relationships.progress_relationship(
                        other_character_id)
                ...
                if character_id not in other_character.relationships:
                    ...
                    other_character.relationships.progress_relationship(
                        character_id)
